...
## Intro
In this lesson, we'll customize the look&feel of the node templates

## What you'll learn
* How to create a custom look of the diagram node
...
GoJS nodes are composed of many GraphObjects such as Panels, Shapes, TextBlocks, etc. As you could already see in the previous lesson, the node definition must start with the `go.Node` declaration.
...
Inside a node, we've added an Automatic Panel that holds the TextBlock displaying the family members' name.


## Panels and Shapes

We want our family tree to look more complex rather than just a simple rectangle with a family name in it. Let's boost our template with a family member photo placeholder and a sleek name container.
...
The prime difference between the Panel Auto and Panel Spot is that the Panel Auto resizes the main element to fit around other elements, whereas the Panel Spot arranges the graph objects around the main element using alignment properties. In our case, to position elements correctly we need to use Panel Spot.

Next, let's add a circle shape (`go.Shape`), which is going to be our placeholder for the family members' photo.
...
All's great, however, it would be better to wrap the textbox with a nicer container.

In order to achieve that, we have to add a new Auto Panel inside our main Panel Spot. It'll contain the Rounded Rectangle Shape (which is going to be a main graph object for the Auto Panel as it's declared first). Underneath, we can place the TextBlock with a family member's name.

As you already know, the Auto Panel resizes the main object to fit around other objects. In our case the Rounded Rectangle will fit around the TextBlock, so we don't need to worry about its size.
...
The `alignmentFocus` property defines the spot on the Graph Object to be used as the alignment point, `alignment` sets the alignment Spot of the Graph Object, so in the other words, it positions the element.

We'll set the `alignment` to `go.Spot.Center` and `alignmentFocus` to `go.Spot.MiddleBottom` which means - I want the middle bottom point of my shape to be placed in the center of the container.
...
So far we've been mostly defining static properties of our nodes. As each node represents a different entity, we want to be able to customize each node based on its data.

Let's catch a glimpse of our data structure:
``` typescript
...
This simple line tells GoJS to set the `text` property of the TextBlock as whatever it finds under the `name` property for each entity.
...
It tells GoJS to apply the color `CssVariable.Gray500` if `deathYear` is defined in the model. Otherwise apply `CssVariable.Purple500`.
...
Let's change the source property to the empty string
...
Setting the source prop to an empty string means that the binding will be fired on every change to the node, hurting the performance.
It's not that noticeable in our case, but having many of these bindings together with lots of nodes on the diagram will end up with the diagram struggling to update every single one of them.
...
First, we need to add the identifier to the family member info panel
...
In order to do so, we need to create the Spot Panel and move the previously defined alignment properties there.
In the panel, there're going to be two elements: the shape and the picture.
...
We can use what we've learned so far about the bindings and extend the photo source binding to show placeholder avatars depending on gender.
...
When a new child is born, we should add a possibility to add a new family member to the diagram. In the `index.ts` add
...
Now that we've added new members to our royal family, we should have a possibility to connect them to their parents.
...
Moreover, we shouldn't allow the child to be linked with more than one parent.

Let's use the `photoPanel` as child port and `familyMemberInfoPanel` as parent port.
...
Last, we need to tell GoJS how the from and to ports are called in our node definition. In order to do so, we need to add two additional properties in our `addModel.ts` file:
...
As we've defined the constraints to our ports, we need to make sure that our data are correctly linked.
To achieve this, extend the link data array:
...
Now, when we double click on the text, the edit box should appear allowing us to enter a name.
Please notice, that we've also added the `.makeTwoWay()` to the binding. Thanks to that, when we save our model, the user input will also be preserved as `makeTwoWay` creates a two-way binding.
In other words, every change to the data object will set the corresponding bound property and vice versa.

![Template 10](../assets/lesson-2/node-template-9.png)

## Summary

In this lesson we've learned how to create customized node templates, bind data and control the linking between elements. In the next lesson, we'll find out how to customize links' appearance and behavior.

## Homework

As homework, display the birth year and death year below the family member's name, separated by a horizontal line. Additionally, display a crown icon above the photo if the reign year is specified in the model. You can find the icon with `import { CROWN_ICON } from "../../consts/icons";`.
